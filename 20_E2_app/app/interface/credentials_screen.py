...
class CredentialsScreen(Screen):
    ...
    def save_credentials(self, instance):
        ...
        if service and service_user and service_password:
            ...
            # Loggear datos cifrados, HMAC y firma
            ...
            db_manager.store_credentials(username, service, encrypted_user, encrypted_password, hmac_user,
                                         hmac_password, salt.decode('utf-8'), signature)
    ...
    def view_credentials(self, instance):
        ...
        if credentials:
            ...
            for credential in credentials:
                ...
                signature = credential[7]

                # Generar clave
                key = generate_key(salt)

                # Verificar HMACs y firma digital
                ...
                toggle_button.bind(on_press=lambda btn, enc_user=encrypted_user, enc_pass=encrypted_password,
                                                   ...
                                                   p_label=password_label, sig=signature_valid,
    ...
    def toggle_visibility(self, encrypted_user, encrypted_password, hmac_user, hmac_password, user_label,
                          password_label, key, salt, button, signature_valid):
        if user_label.text == "****" and password_label.text == "****":
            ...
            if user_verification and password_verification and signature_valid:
                ...
                logger.log_decrypted_data("Username", decrypted_user)
                logger.log_decrypted_data("Password", decrypted_password)

                user_label.text = decrypted_user
                password_label.text = decrypted_password
                button.text = "Hide"

                logger.log_signature_verification("Digital Signature", True)
            else:
                logger.log_signature_verification("Digital Signature", False)
                show_popup("Error", "Data integrity check failed.")
        else:
            user_label.text = "****"
            password_label.text = "****"
            button.text = "Show"
